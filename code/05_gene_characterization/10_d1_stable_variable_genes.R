...
exp.tpm<-read.delim("all_sample_tpm.protein_coding.txt",header=T,row.names = 1)
...
########5. 轨迹分析图（适合时间序列数据）:展示基因表达随时间的变化轨迹
# 使用splines或loess拟合轨迹
library(splines)

# 对每个个体绘制关键基因的轨迹
key_genes <- c("GAPDH", "MYC", "ACTB")  # 示例基因

key_genes <- c("ENSG00000142611", "ENSG00000142655", "ENSG00000142583")  # 示例基因

trajectory_data <- data.frame(
  Subject = rep(type$Donor, each = length(key_genes)),
  Gene = rep(key_genes, times = nrow(type)),
  TimePoint = rep(type$Time, each = length(key_genes)),
  Expression = as.vector(t(logCPM[key_genes, ]))
)

ggplot(trajectory_data, aes(x = TimePoint, y = Expression, group = Subject)) +
  geom_line(alpha = 0.3) +
  stat_summary(aes(group = 1), fun = mean, geom = "line", 
               color = "red", size = 1.5) +  # 平均轨迹
  facet_wrap(~ Gene, scales = "free_y", ncol = 3) +
  labs(x = "Time Point", y = "log2(Expression)") +
  theme_bw()
####6. 方差分解图:定量展示个体间 vs 个体内变异占比
# 使用方差成分分析
library(lme4)
head(type)
# 对每个基因拟合混合效应模型，估计方差成分
var_components <- apply(logCPM, 1, function(y) {
  tryCatch({
    model <- lmer(y ~ Time + (1|Donor), data = type)
    vc <- as.data.frame(VarCorr(model))
    return(c(Subject_var = vc$vcov[1], Residual_var = vc$vcov[2]))
  }, error = function(e) return(c(Subject_var = NA, Residual_var = NA)))
})

var_components <- t(var_components)
var_components <- na.omit(var_components)

# 计算平均方差占比
avg_subject_var <- mean(var_components[, "Subject_var"] / 
                          rowSums(var_components))

# 绘制方差分布
ggplot(data.frame(Proportion = var_components[, "Subject_var"] / 
                    rowSums(var_components)), 
       aes(x = Proportion)) +
  geom_histogram(bins = 30, fill = "steelblue", alpha = 0.7) +
  geom_vline(xintercept = avg_subject_var, color = "red", linetype = "dashed") +
  labs(x = "Proportion of variance explained by subject",
       y = "Number of genes",
       title = paste0("Average subject variance: ", 
                      round(100*avg_subject_var, 1), "%")) +
  theme_minimal()
#####7. 网络图（样本相似性网络）
library(igraph)
library(qgraph)
...
diag(adj_matrix) <- 0  # 去除对角线
adj_matrix[adj_matrix < 0.7] <- 0  # 设置阈值

# 创建网络图
network <- qgraph(adj_matrix,
                  layout = "spring",
                  groups = list(Donor = type$Donor),
                  color = brewer.pal(4, "Set1"),
                  vsize = 8,
                  edge.color = "gray50",
                  title = "Sample similarity network")
...
head(m1)
write.table(m1,
            "data_mean.add_sd.TPM.12D1samples.protein_coding.txt", 
            sep="\t", col.names=T,quote = F,row.names = F)
head(m1)
...
gene_stats.D1<-gene_stats
save(gene_stats.D1,file="gene_stats.D1.Rdata")

gene_stats<-gene_stats[gene_stats$mean_expression>5,]
  # 按CV从大到小排序
  ...
  p1 <- ggplot(gene_stats, aes(x = log10(median_expression + 1), y = cv_expression)) +
    ...
    theme_minimal()
  print(p1)
  ggsave(filename="./point.Gene_Expression_Variability_Across_12_Donors.pdf",plot=p1,
...
####第二部分：对基因进行三级分类:我们将基于CV的分布，客观地将基因分为“稳定”、“中等变异”、“高变异”三类
  ...
  ggsave(filename="./violin.Gene_Expression_Variability_Distribution_Across_Variability_Classes.pdf",plot=p2,
         device='pdf',path=".",width=3.5,height=3.5)
#####第三部分：探索不同类别基因的特征
  # 5. 提取各类基因列表（用于后续功能分析）
  gene_stats<-gene_stats[gene_stats$mean_expression>5,]
  ...
  write.table(v1,file='gene_stats_sorted.high_var.txt',sep='\t',quote=FALSE,row.names=FALSE,col.names=TRUE)
  write.table(v2,file='gene_stats_sorted.moderate.txt',sep='\t',quote=FALSE,row.names=FALSE,col.names=TRUE)
  write.table(v3,file='gene_stats_sorted.stable.txt',sep='\t',quote=FALSE,row.names=FALSE,col.names=TRUE)
  ...
  ggsave(filename = "GO.Highly_Variable_Genes.higher5.pdf", 
         plot =p,width = 11, height = 8, units = 'cm')
  ...
  write.table(m1,file='top50gene_stats_sorted.high_var.txt',sep='\t',
              quote=FALSE,row.names=TRUE,col.names=TRUE)
  # 对基因和样本进行聚类
  pdf("protein_coding.Highly_Variable_Genes.top50.pdf",4.6,3.6)
  ...
  pdf("protein_coding.Highly_Variable_Genes.top50.new.pdf",4,4)
  mycol <- colorRamp2(c(-2, 0, 2), c("blue", "white", "red"))
  ...
  write.table(m1,file='top50gene_stats_sorted.moderate.txt',sep='\t',
              quote=FALSE,row.names=TRUE,col.names=TRUE)
  
  # 对基因和样本进行聚类
  pdf("protein_coding.moderate_Variable_Genes.top50.pdf",4.6,3.6)
  pheatmap(log2(heatmap_data + 1),
           scale = "row", # 按行标准化，突出基因在不同样本间的模式
           clustering_distance_rows = "euclidean",
           clustering_distance_cols = "euclidean",
  ...
  pdf("protein_coding.moderate_Variable_Genes.top50.new.pdf",4,4)
  mycol <- colorRamp2(c(-2, 0, 2), c("blue", "white", "red"))
  data.m<-na.omit(data.m)
  Heatmap(as.matrix(data.m),cluster_columns = TRUE,cluster_rows = TRUE,
          col=mycol,name = "Exp\nZ score", 
  ...
  write.table(m1,file='top50gene_stats_sorted.stable.txt',sep='\t',
              quote=FALSE,row.names=TRUE,col.names=TRUE)
  # 对基因和样本进行聚类
  pdf("protein_coding.stable_Genes.top50.pdf",4.6,3.6)
  pheatmap(log2(heatmap_data + 1),
           scale = "row", # 按行标准化，突出基因在不同样本间的模式
           clustering_distance_rows = "euclidean",
           clustering_distance_cols = "euclidean",
  ...
  pdf("protein_coding.stable_Genes.top50.new.pdf",4,4)
  mycol <- colorRamp2(c(-2, 0, 2), c("blue", "white", "red"))
  data.m<-na.omit(data.m)
  Heatmap(as.matrix(data.m),cluster_columns = TRUE,cluster_rows = TRUE,
          col=mycol,name = "Exp\nZ score", 
  ...
  dev.off()
  ######################################################
  #基于功能富集的“锚定”法（最严谨）选择CV阈值筛选出稳定表达基因：
  #通过生物学功能来反推合理的阈值。其逻辑是：逐步收紧CV阈值，
  #直到筛选出的基因集合显著富集已知的“看家”或“核心”功能通路。
  #  计算每个基因的关键统计量
  gene_stats <- data.frame(gene = rownames(tpm_matrix),
                           median_expression = apply(tpm_matrix, 1, median), # 中位表达水平
                           mean_expression = apply(tpm_matrix, 1, mean),
  ...
  cv_thresholds <- seq(30, 100, by = 2.5)  # 测试从5%到30%的阈值
  enrichment_results <- list()
  
  for (thresh in cv_thresholds) {
    stable_set <- gene_stats[gene_stats$cv_expression < thresh,]
    
    stable_set_entrez<- bitr(stable_set$gene, fromType = "ENSEMBL", 
                            toType = "ENTREZID", 
                            OrgDb = org.Hs.eg.db)$ENTREZID
    # 2. 对每个基因集进行GO“看家功能”富集分析（例如，核糖体、氧化磷酸化）
    # 此处需要将基因符号转换为Entrez ID，略去转换代码
    go_res <- enrichGO(gene = stable_set_entrez, 
                       OrgDb = org.Hs.eg.db,
                       ont = "BP",
                       pvalueCutoff = 0.01)
    
    # 3. 计算一个“看家特征分数”（如：在核糖体通路富集分析的 -log10(p值)）
    # 假设我们关注“核糖体组装”(GO:0042255)这个术语
    ribosome_pval <- ifelse("GO:0042255" %in% go_res$ID, 
                            -log10(go_res$pvalue[go_res$ID == "GO:0042255"]), 
                            0)
    enrichment_results[[as.character(thresh)]] <- ribosome_pval
  }
  
  # 4. 绘制“CV阈值” vs “看家特征分数”曲线，选择分数开始进入平台期的拐点
  plot(cv_thresholds, unlist(enrichment_results), type = "b", 
       xlab = "CV Threshold (%)", ylab = "Ribosome Pathway Enrichment (-log10 p-value)")
#unlist(enrichment_results) 最后稳定表达基因CV阈值选定为62.5
#############################################################################
####计算高中低变异基因中高中低表达基因数目百分比pie图
  ...
  #Stable                    470               5057            1000
  #Moderate                 1568               5127              30
  #Highly Variable          2371               4101              55 
...
##第二步：绘制表达水平与变异程度的分布图
  ...
  print(p_expr_vs_cv)
  ggsave("./boxplot_CV_vs_Expression_Level.pdf", p_expr_vs_cv, 
  ...
  print(p_scatter)
  ggsave("./scatter_Expression_vs_CV_by_Level.pdf", p_scatter, 
...
####第三步：制作饼图展示百分比分布
  ...
  # 保存组合图形
  ggsave("./pie_Expression_Level_Distribution_by_Variability.pdf", 
...
####第四步：制作堆叠条形图（备选，更易比较）
  ...
  print(p_stacked)
  ggsave("./stacked_bar_Expression_Level_Distribution.pdf", p_stacked, 
...
####第五步：创建汇总表格
  ...
  # 保存为CSV文件
  write.csv(summary_table, "./summary_Expression_Variability_Table.csv", row.names = FALSE)
...
##########heatmap D1 sample 最开始版本，未按照主任意思修改前的分析和图
