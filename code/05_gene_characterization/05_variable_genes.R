# =============================================================================
# 05_variable_genes
# Description: Highly Variable Genes Analysis
# =============================================================================
...
#########################热图
####protein coding
gene<-read.delim("2888gene.txt",header=T,row.names = 1)
phenotype.1
df1<-df.pro[gene$ENSEMBL,phenotype.1$sample]
...
pdf(file = "./heatmap.pro.2888gene.expTPM.noscaled.pdf", 5.5, 5)
...
pdf(file = "./heatmap.pro.2888gene.expTPM.scaled.pdf", 5.5, 5)
...
p1
ggsave(filename="./boxplot.bydonor.2888gene.log2TPM.addP.4.pdf",plot=p1,
...
test3<-ggplot(exp_melt.final.1, aes(x=tpm, color=Time)) +
  ...
  labs(title="Protein coding")
test3
ggsave(filename="density.plot.2888gene.byDonor.plot.tpm.pdf",plot=test3,
