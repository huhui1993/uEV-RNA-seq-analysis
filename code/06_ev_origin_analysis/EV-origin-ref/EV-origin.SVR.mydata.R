...
setwd("D:\\项目\\231021661.报告--尿液外泌体转录组\\自己重新跑后\\EV-origin\\EV-origin-master\\EV-origin-master")
#library(e1071)
### nu-SVR
### read the reference matrix as "ref.m"
list.files()
expr<-read.delim("all_sample_tpm.txt",header=T,row.names = 1)
head(expr);dim(expr)
...
expr_numeric <- expr
expr_numeric[, -1] <- apply(expr_numeric[, -1], 2, function(x) as.numeric(x))

# 检查转换后的结构
str(expr_numeric)

test1<-aggregate(x=expr_numeric[,2:31],by=list(expr_numeric$gene_name),FUN=mean,na.rm=T) 
dim(test1)
...
write.table(test1,"exp.tpm.for_tissue_origin.txt", 
            sep="\t", col.names=T,quote = F)

exp<-expr_numeric[,c(2:31)]
...
load("exLR_origin.results.cell.Rdata")

head(exLR_origin.results)
head(exLR_origin.results.cell)
