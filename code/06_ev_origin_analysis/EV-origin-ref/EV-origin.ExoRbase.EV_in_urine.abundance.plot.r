...
####################################################################
setwd("D:\\项目\\231021661.报告--尿液外泌体转录组\\自己重新跑后\\EV-origin\\EV-origin-master\\EV-origin-master")
results<-read.delim("ExoRbase3.0.EVPs_in_Urine_origins_abs.txt",header=T,row.names = 1)
dim(results)
head(results)
...
ggsave(filename="barplot_ExoRbase3.0.EVPs_in_Urine_origins_Rabs.64_125.pdf",plot=p.final,
       device='pdf',path=".",width=14,height=5)
####D1 sample
head(c)
