...
colnames(exp_melt)<-c("geneid","sample","tpm")
head(exp_melt);dim(exp_melt)
exp_melt.final<-merge(exp_melt,phenotype.d6,by = "sample",all=T)
head(exp_melt.final)
...
p <- levelplot(x[row.ord, col.ord],
               ...
               main="All Donor6 samples TPM correlation",
               legend =
                 list(right =
...
# output
file <- paste("./donor6", "/all_sample_cluster.donor6.png", sep="")
png(filename=file, height = 1200, width = 1200, res = 300, units = "px")
print(p)
dev.off()

file <- paste("./donor6", "/all_sample_cluster.donor6.pdf", sep="")
...
#################################
####相关性画图
file <- paste("./donor6", "/all_sample_pearson.cor.donor6.xls", sep="")
...
file <- paste("./donor6", "/all_donor6sample_cor.png", sep="")
...
pairs(log10(data1+1), pch = ".", upper.panel = panel.cor, diag.panel = panel.hist, 
      lower.panel = panel.lm, main = "Pearson Correlation of Donor6 samples",gap=0)
dev.off()


file <- paste("./donor6", "/all_donor6sample_cor.pdf", sep="")
pdf(file=file, height = 3, width = 3)
pairs(log10(data1+1), pch = ".", upper.panel = panel.cor, diag.panel = panel.hist, 
      lower.panel = panel.lm, main = "Pearson Correlation of all Donor6 samples",gap=0)
dev.off()
...
test
ggsave(filename="./Donor6/density.plot.all_Donor6sample.tpm.pdf",plot=test,
...
test
ggsave(filename="./donor6/density.plot.protein_coding.all_donor6sample.tpm.pdf",plot=test,
       device='pdf',path=".",width=4.5,height=2.8)
###by time
head(exp_melt);dim(exp_melt)
head(exp_melt.final)
...
df1<-df.lnc[,phenotype.d6$sample]
...
test<-ggplot(exp_melt.final, aes(x=log10tmp, color=sample)) +
  ...
  labs(title="LncRNA")
test
ggsave(filename="./donor6/density.plot.lncrna.all_donor6sample.tpm.pdf",plot=test,
       device='pdf',path=".",width=4.5,height=2.8)
###by time
head(exp_melt);dim(exp_melt)
head(exp_melt.final)
...
head(all_result)
save(all_result,file="./donor6/all_result.RData")
write.table(all_result,
            "./donor6/all_result.donor6.sample.low.medium.high.gene.count.txt", 
...
colnames(all_result)<-c("low","medium","high")
head(all_result)
save(all_result,file="./donor6/all_result.pro.RData")
write.table(all_result,
            "./donor6/all_result.protein_coding.donor6.sample.low.medium.high.gene.count.txt", 
            sep="\t", col.names=T,quote = F)
################lnc
exp.tpm.pro<-subset(exp.tpm,gene_biotype %in% c("protein_coding"))
exp.tpm.lnc<-subset(exp.tpm,gene_biotype %in% c("lncRNA"))
...
colnames(all_result)<-c("low","medium","high")
head(all_result)
save(all_result,file="./donor6/all_result.lnc.RData")
write.table(all_result,
            "./donor6/all_result.lnc.donor6.sample.low.medium.high.gene.count.txt", 
            sep="\t", col.names=T,quote = F)
##################################################################
##山脊线图
