...
phenotype.d4<-phenotype %>% subset(Donor=="4")
####Donor4 exp matrix
df1<-df[,phenotype.d4$sample]
...
colnames(exp_melt)<-c("geneid","sample","tpm")
head(exp_melt);dim(exp_melt)
exp_melt.final<-merge(exp_melt,phenotype.d4,by = "sample",all=T)
head(exp_melt.final)
...
p <- levelplot(x[row.ord, col.ord],
               ...
               main="All donor4 samples TPM correlation",
               legend =
                 list(right =
...
# output
file <- paste("./donor4", "/all_sample_cluster.donor4.png", sep="")
png(filename=file, height = 1800, width = 1800, res = 300, units = "px")
print(p)
dev.off()

file <- paste("./donor4", "/all_sample_cluster.donor4.pdf", sep="")
...
#################################
####相关性画图
file <- paste("./donor4", "/all_sample_pearson.cor.donor4.xls", sep="")
...
file <- paste("./donor4", "/all_donor4sample_cor.png", sep="")
...
pairs(log10(data1+1), pch = ".", upper.panel = panel.cor, diag.panel = panel.hist, 
      lower.panel = panel.lm, main = "Pearson Correlation of donor4 samples",gap=0)
dev.off()


file <- paste("./donor4", "/all_donor4sample_cor.pdf", sep="")
pdf(file=file, height = 3, width = 3)
pairs(log10(data1+1), pch = ".", upper.panel = panel.cor, diag.panel = panel.hist, 
      lower.panel = panel.lm, main = "Pearson Correlation of all donor4 samples",gap=0)
dev.off()
...
test
ggsave(filename="./donor4/density.plot.all_donor4sample.tpm.pdf",plot=test,
...
test
ggsave(filename="./donor4/density.plot.protein_coding.all_donor4sample.tpm.pdf",plot=test,
       device='pdf',path=".",width=4.5,height=2.8)
###by time
head(exp_melt);dim(exp_melt)
head(exp_melt.final)
...
df1<-df.lnc[,phenotype.d4$sample]
...
test<-ggplot(exp_melt.final, aes(x=log10tmp, color=sample)) +
  ...
  labs(title="LncRNA")
test
ggsave(filename="./donor4/density.plot.lncrna.all_donor4sample.tpm.pdf",plot=test,
       device='pdf',path=".",width=4.5,height=2.8)
###by time
head(exp_melt);dim(exp_melt)
head(exp_melt.final)
...
head(all_result)
save(all_result,file="./donor4/all_result.RData")
write.table(all_result,
            "./donor4/all_result.donor4.sample.low.medium.high.gene.count.txt", 
...
colnames(all_result)<-c("low","medium","high")
head(all_result)
save(all_result,file="./donor4/all_result.pro.RData")
write.table(all_result,
            "./donor4/all_result.protein_coding.donor4.sample.low.medium.high.gene.count.txt", 
            sep="\t", col.names=T,quote = F)
################lnc
exp.tpm.pro<-subset(exp.tpm,gene_biotype %in% c("protein_coding"))
exp.tpm.lnc<-subset(exp.tpm,gene_biotype %in% c("lncRNA"))
...
colnames(all_result)<-c("low","medium","high")
head(all_result)
save(all_result,file="./donor4/all_result.lnc.RData")
write.table(all_result,
            "./donor4/all_result.lnc.donor4.sample.low.medium.high.gene.count.txt", 
            sep="\t", col.names=T,quote = F)
##################################################################
##山脊线图
