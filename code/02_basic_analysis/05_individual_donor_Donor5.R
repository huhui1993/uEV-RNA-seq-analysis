...
phenotype.d5<-phenotype %>% subset(Donor=="5")
####donor5 exp matrix
df1<-df[,phenotype.d5$sample]
...
exp_melt.final<-merge(exp_melt,phenotype.d5,by = "sample",all=T)
head(exp_melt.final)
...
p <- levelplot(x[row.ord, col.ord],
               ...
               main="All donor5 samples TPM correlation",
               legend =
                 list(right =
...
# output
file <- paste("./donor5", "/all_sample_cluster.donor5.png", sep="")
png(filename=file, height = 1800, width = 1800, res = 300, units = "px")
print(p)
dev.off()

file <- paste("./donor5", "/all_sample_cluster.donor5.pdf", sep="")
...
#################################
####相关性画图
file <- paste("./donor5", "/all_sample_pearson.cor.donor5.xls", sep="")
...
file <- paste("./donor5", "/all_donor5sample_cor.png", sep="")
...
pairs(log10(data1+1), pch = ".", upper.panel = panel.cor, diag.panel = panel.hist, 
      lower.panel = panel.lm, main = "Pearson Correlation of donor5 samples",gap=0)
dev.off()


file <- paste("./donor5", "/all_donor5sample_cor.pdf", sep="")
pdf(file=file, height = 3, width = 3)
pairs(log10(data1+1), pch = ".", upper.panel = panel.cor, diag.panel = panel.hist, 
      lower.panel = panel.lm, main = "Pearson Correlation of all donor5 samples",gap=0)
dev.off()
...
test
ggsave(filename="./donor5/density.plot.all_donor5sample.tpm.pdf",plot=test,
...
test
ggsave(filename="./donor5/density.plot.protein_coding.all_donor5sample.tpm.pdf",plot=test,
       device='pdf',path=".",width=3.5,height=2)
###by time
head(exp_melt);dim(exp_melt)
head(exp_melt.final)
...
df1<-df.lnc[,phenotype.d5$sample]
...
test<-ggplot(exp_melt.final, aes(x=log10tmp, color=sample)) +
  ...
  labs(title="LncRNA")
test
ggsave(filename="./donor5/density.plot.lncrna.all_donor5sample.tpm.pdf",plot=test,
       device='pdf',path=".",width=3.5,height=2)
###by time
head(exp_melt);dim(exp_melt)
head(exp_melt.final)
...
head(all_result)
save(all_result,file="./donor5/all_result.RData")
write.table(all_result,
            "./donor5/all_result.donor5.sample.low.medium.high.gene.count.txt", 
...
colnames(all_result)<-c("low","medium","high")
head(all_result)
save(all_result,file="./donor5/all_result.pro.RData")
write.table(all_result,
            "./donor5/all_result.protein_coding.donor5.sample.low.medium.high.gene.count.txt", 
            sep="\t", col.names=T,quote = F)
################lnc
exp.tpm.pro<-subset(exp.tpm,gene_biotype %in% c("protein_coding"))
exp.tpm.lnc<-subset(exp.tpm,gene_biotype %in% c("lncRNA"))
...
colnames(all_result)<-c("low","medium","high")
head(all_result)
save(all_result,file="./donor5/all_result.lnc.RData")
write.table(all_result,
            "./donor5/all_result.lnc.donor5.sample.low.medium.high.gene.count.txt", 
            sep="\t", col.names=T,quote = F)
##################################################################
##山脊线图
