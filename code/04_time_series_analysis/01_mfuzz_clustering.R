# =============================================================================
# 08. Time Series Clustering with Mfuzz
# Description: Fuzzy C-means clustering of genes across 4 time points
# Input: all_sample_count.txt, sample_info.txt
# Output: Mfuzz cluster plots (PDF)
# =============================================================================
...
identical(colnames(df),rownames(phenotype))
head(df);dim(df)

colnames(df)<-phenotype$Time

dat <- log2(edgeR::cpm(df)+1)
dat[1:4,1:4]
dim(dat)
...
avereps_df[1:4,1:4]
colnames(avereps_df)
save(avereps_df,file = 'avereExp_log2CPM.count.Rdata')
...
# 1765 genes excluded ，不同的数据集去除的基因数量不一样
eset
...
avereps_df[1:4,1:4]
colnames(avereps_df)
save(avereps_df,file = 'avereExp_log2CPM.count.male.Rdata')
## 2.1 Filtering----
# 去除表达量太低或者在不同时间点间变化太小的基因等步骤
# Mfuzz聚类时要求是一个ExpressionSet类型的对象，所以需要先用表达量构建这样一个对象。
eset <- new("ExpressionSet",exprs = avereps_df)
# 根据标准差去除样本间差异太小的基因
eset <- filter.std(eset,min.std=0)
# 2266 genes excluded ，不同的数据集去除的基因数量不一样
eset
...
# coupling between clusters
overlap.cl <- overlap(cl)
pdf('mfuzz_overlap_plot.male.pdf',height = 4,width = 5)
p.overlaps <- overlap.plot(cl, over = overlap.cl, thres = 0.05)
p.overlaps
dev.off()
...
library(RColorBrewer)
color.2 <- colorRampPalette(rev(c("#ff0000", "Yellow", "OliveDrab1")))(1000)
pdf('mfuzz_clusters_plot.male.pdf',height = 7,width = 12)
mfuzz.plot(eset,cl,mfrow=c(3,3),
           new.window= FALSE,
           time.labels= colnames(eset) ,
           colo = color.2)
dev.off()
pdf('mfuzz_clusters_plot01.male.pdf',height = 7,width = 12)
mfuzz.plot2(eset, cl, mfrow = c(3, 3),
            time.labels= colnames(eset) ,
            centre = T, x11 = F, centre.lwd = 0.2)
dev.off()
...
# 3. 批量输出聚类所含的基因 
dir.create(path = "mfuzzGenes.male", recursive = T)
for (i in 1:5) {
  potname = names(cl$cluster[unname(cl$cluster) == i])
  write.csv(cl[[4]][potname, i], paste0("mfuzzGenes.male", "/mfuzz_male_", i, ".csv"))
}
...
dat <- log2(edgeR::cpm(df.female)+1)
dat[1:4,1:4]
dim(dat)
...
avereps_df[1:4,1:4]
colnames(avereps_df)
save(avereps_df,file = 'avereExp_log2CPM.count.female.Rdata')
## 2.1 Filtering----
# 去除表达量太低或者在不同时间点间变化太小的基因等步骤
# Mfuzz聚类时要求是一个ExpressionSet类型的对象，所以需要先用表达量构建这样一个对象。
eset <- new("ExpressionSet",exprs = avereps_df)
...
# coupling between clusters
overlap.cl <- overlap(cl)
pdf('mfuzz_overlap_plot.female.pdf',height = 4,width = 5)
p.overlaps <- overlap.plot(cl, over = overlap.cl, thres = 0.05)
p.overlaps
dev.off()
...
library(RColorBrewer)
color.2 <- colorRampPalette(rev(c("#ff0000", "Yellow", "OliveDrab1")))(1000)
pdf('mfuzz_clusters_plot.female.pdf',height = 7,width = 12)
mfuzz.plot(eset,cl,mfrow=c(3,3),
           new.window= FALSE,
           time.labels= colnames(eset) ,
           colo = color.2)
dev.off()
pdf('mfuzz_clusters_plot01.female.pdf',height = 7,width = 12)
mfuzz.plot2(eset, cl, mfrow = c(3, 3),
            time.labels= colnames(eset) ,
            centre = T, x11 = F, centre.lwd = 0.2)
dev.off()
...
# 3. 批量输出聚类所含的基因 
dir.create(path = "mfuzzGenes.female", recursive = T)
for (i in 1:5) {
  potname = names(cl$cluster[unname(cl$cluster) == i])
  write.csv(cl[[4]][potname, i], paste0("mfuzzGenes.female", "/mfuzz_female_", i, ".csv"))
}
######
####################数据基本分析
library(Mfuzz)
