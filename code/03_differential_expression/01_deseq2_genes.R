# =============================================================================
# 05. Differential Expression Analysis (DESeq2) - Protein-coding Genes
# Description: Identify DEGs between time points (D15/D29/D43 vs D1)
# Input: all_sample_count.addAnotation.txt, sample_info.txt
# Output: DESeq2 result tables (.xls)
# =============================================================================
...
data.exp<-expr.pro[,c(phenotype$sample)]
...
head(res_addMean)
#####输出未筛选的差异结果
write.table(res_addMean,file=paste("gene.DESeq2.",combn(list, 2)[,i][2],"vs",combn(list, 2)[,i][1],".compare.nofilter.xls",sep = ""),
...
##输出筛选后的差异结果
write.table(diff_gene_deseq2,file=paste("gene.DESeq2.",combn(list, 2)[,i][2],"vs",combn(list, 2)[,i][1],".compare.p0.05.FC1.5.Higher5.xls",sep = ""),
            quote = F,sep='\t',row.names=T,col.names =NA )

diff_gene_deseq2 <-subset(deseq_gene, pvalue < 0.05 & abs(log2FoldChange) > 1)
dim(diff_gene_deseq2)
##输出筛选后的差异结果
write.table(diff_gene_deseq2,file=paste("gene.DESeq2.",combn(list, 2)[,i][2],"vs",combn(list, 2)[,i][1],".compare.p0.05.FC2.Higher5.xls",sep = ""),
            quote = F,sep='\t',row.names=T,col.names =NA )
}
