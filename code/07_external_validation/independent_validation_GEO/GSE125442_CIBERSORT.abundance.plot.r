...
setwd("D:\\项目\\231021661.报告--尿液外泌体转录组\\自己重新跑后\\EV-origin\\独立验证集GEO\\GSE125442")
list.files()
results<-read.delim("GSE125442_CIBERSORT-Results.txt",header=T,row.names = 1)
dim(results)
head(results)
...
ggsave(filename="barplot_uEV_GSE125442_CIBERSORT-Results.tissue.pdf",plot=p.final,
       device='pdf',path=".",width=5,height=4.5)
####D1 sample
head(c)
...
type_medians <- c %>%
  ...
  arrange(desc(median_abundance))  # 按中位数降序排列

# 重新设置Type因子的水平，按照中位数从大到小排列
c$Type <- factor(c$Type, levels = type_medians$Type)
...
ggsave(filename="boxplot_EV_CIBERSORT.abundance.pdf",plot=p,
       device='pdf',path=".",width=6,height=3)
